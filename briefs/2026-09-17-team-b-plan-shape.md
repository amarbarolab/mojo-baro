# Team B conference: how do you two want the platform plan shaped?

You are one of two peers of equal standing on the same tree: `sonnet` (Claude Sonnet, pane `w8M:p1`)
and `codex` (pane `w8M:p2`). The coordinator is pane `w82:pC`. Repo: `~/Projects/mojo/mojo-baro`
...
The room lives in the Brain: `$HOME/Brain/mojo/mojo-baro/rooms/B/` (`room.md` chat, `cards.md` cards).
Always by absolute path, from the repo directory:

    ~/iTools/bin/room say B <your-name> "text"      # appends and pings your partner's pane
    ~/iTools/bin/room read B <your-name>            # what you have not seen yet
    ~/iTools/bin/room cards B
    ~/iTools/bin/room card B <your-name> "<match>" doing|done "<receipt, required for done>"
...
One file, written jointly: `exchange/2026-09-17-team-B-plan-shape.md`, sections 1 to 7 as above,
...
(`signed: sonnet`, `signed: codex`), sonnet commits it by pathspec (no attribution lines) and runs:
    ...
    ~/iTools/bin/herd tell w82:pC "team B plan shape written to exchange/2026-09-17-team-B-plan-shape.md"

Then stop. Do not start building.

## Addenda (both teams)

- **You stay on for the build.** The same two agents that sign this document build the items, so
  shape the plan for yourselves, not for a stranger.
- **Card 8, from the maintainer: iTools wishlist.** Which tools would make you faster or more reliable
  (build, gate, search, review, handoff)? Browse `~/iTools/INDEX.md` first; for each wish give the
  command shape and the time or failure it removes. Section 8 of the deliverable.
- Team A (sonnet `w8K:p1`, codex `w8K:p2`) runs the same conference in room A
  (`$HOME/Brain/mojo/mojo-baro/rooms/A/room.md`, readable). You may read their room; talk to
  them only through the coordinator. Your worked example for section 2 is P4, not P0a.
