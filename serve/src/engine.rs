...
impl Engine {
    ...
    pub fn submit(
        ...
        let job = Job {
            req: Request { id, prompt, n, spec, stop, ckpt, state_save: state.0, state_load: state.1, sample },
            out,
        };
...
impl EnginePool {
    ...
    pub fn submit(
        ...
        let rx = self.engines[best].submit(id, prompt, n, spec, stop, ckpt, sample, state)?;
        Ok((id, rx))
    }
