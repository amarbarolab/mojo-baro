# bench-local copy of Pack/load_pack/alloc_bufs.
#
# serve/harness.mojo (which extracted these verbatim out of serve/engine.mojo's
# main(), commit 5a823375) is not on this branch's history -- it landed on a
# sibling lane. HARNESS's non-goals forbid touching serve/ except the realign
# stub, so this is a bench/-local duplicate of the same known-working code
# (identical against this branch's serve/window.mojo / serve/registry.mojo,
# diffed byte-for-byte where it matters) rather than a serve/harness.mojo add.
# Once the real serve/harness.mojo lands, this file's contents are that file
# and bench_latent_handoff.mojo's import line is the only thing that changes.
...
def load_pack(ctx: DeviceContext, packdir: String) raises -> Pack:
    ...
    print("loading pack:", total, "bytes from", packdir)
