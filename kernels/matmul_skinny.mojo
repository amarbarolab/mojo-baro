...
def amar_matmul_skinny_m1_dual[
    ...
    A: TileTensor[in_dtype, ALayout, MutAnyOrigin],
    B1: TileTensor[in_dtype, BLayout, MutAnyOrigin],
    B2: TileTensor[in_dtype, BLayout, MutAnyOrigin],
    Cp1: TileTensor[dtype, PLayout, MutAnyOrigin],
    Cp2: TileTensor[dtype, PLayout, MutAnyOrigin],
    m: Int32,
    n: Int32,
    k_dim: Int32,
):
    comptime assert A.flat_rank == 2 and B1.flat_rank == 2 and Cp1.flat_rank == 3
    ...
    var B1v = B1.vectorize[1, CPT]()
    var B2v = B2.vectorize[1, CPT]()

    var acc1 = SIMD[dtype, CPT](0)
    var acc2 = SIMD[dtype, CPT](0)
    ...
    while kk < k1:
        ...
        if c0 < N:
            for k in range(clen):
                var a_val = rebind[Scalar[dtype]](sa[k])
                var b1 = rebind[SIMD[in_dtype, CPT]](
                    B1v[kk + k, c0 // CPT]
                ).cast[dtype]()
                var b2 = rebind[SIMD[in_dtype, CPT]](
                    B2v[kk + k, c0 // CPT]
                ).cast[dtype]()
                acc1 += b1 * a_val
                acc2 += b2 * a_val
    ...
    if c0 < N:
        comptime for j in range(CPT):
            Cp1[block_idx.y, 0, c0 + j] = rebind[Cp1.ElementType](acc1[j])
            Cp2[block_idx.y, 0, c0 + j] = rebind[Cp2.ElementType](acc2[j])
